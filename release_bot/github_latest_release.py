...
GITHUB_LATEST_RELEASE_FORMAT_STR: str = (
...
def _github_repo_url_to_latest_release_api_url(repo_url: str) -> str:
    ...
    if url_parts.netloc != "github.com":
        raise ValueError(
            "This method only supports Github URLS, got {}".format(url_parts.netloc)
        )
    path_parts = os.path.split(url_parts.path)
    if len(path_parts) != 2:
        raise ValueError(
            "The provided url, {}, does not appear to point to a repository.".format(
                repo_url
            )
        )
